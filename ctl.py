...
def socket_input_thread():
    ...
    while g_running:
        ...
        if len(s) > 2:
            if s[0] == 'a':
                if s[1] == '1':
                    os.system("/opt/app/gateway/scripts/rla -l 1")
                    if g_verbose:
                        print "relay A enable"
                else: 
                    os.system("/opt/app/gateway/scripts/rla -l 0")
                    if g_verbose:
                        print "relay A disable"
            elif s[0] == 'b':
                if s[1] == '1':
                    os.system("/opt/app/gateway/scripts/rlb -l 1")
                    
                    if g_verbose:
                        print "relay B enable"
                else :
                    os.system("/opt/app/gateway/scripts/rlb -l 0")
                    if g_verbose:
